    ...
    pub async fn log_position(&self, position: &TradePosition) {
        ...
        if let Some(db) = self.transaction_log.get_db().await {
            let position_log = PositionLog {
                ...
                fee: position.fee(),
                debug: DebugLog {
                    input_1: position.atr().0.round_dp(4),
                    input_2: position.atr().1.round_dp(4),
                    input_3: position.atr().2.round_dp(4),
                    input_4: position.rsi().0.round_dp(4),
                    input_5: position.rsi().1.round_dp(4),
                    input_6: position.rsi().2.round_dp(4),
                    input_7: position.macd().round_dp(4),
                    input_8: position.price_variance().round_dp(4),
                    input_9: position.take_profit_ratio().round_dp(4),
                    input_10: position.atr_spread().round_dp(4),
                    input_11: position.max_open_duration().into(),
                    input_12: position.risk_reward().round_dp(4),
                    input_13: Decimal::ZERO,
                    input_14: Decimal::ZERO,
