        ...
        if let Some(db) = self.transaction_log.get_db().await {
            let position_log = PositionLog {
                ...
                debug: DebugLog {
                    ...
                    input_5: position.rsi().0.round_dp(4),
                    input_6: position.rsi().1.round_dp(4),
                    input_7: position.rsi().2.round_dp(4),
                    input_8: position.rsi().3.round_dp(4),
                    input_9: position.price().3.round_dp(4),
                    input_10: position.price_variance().round_dp(4),
                    input_11: position.take_profit_ratio().round_dp(4),
