        ...
        if let Some(db) = self.transaction_log.get_db().await {
            let position_log = PositionLog {
                ...
                debug: DebugLog {
                    ...
                    input_10: position.macd().0.round_dp(4),
                    input_11: position.macd().1.round_dp(4),
                    input_12: position.macd().2.round_dp(4),
                    input_13: position.stochastic().0.round_dp(4),
                    input_14: position.stochastic().1.round_dp(4),
                    input_15: position.stochastic().2.round_dp(4),
                    input_16: position.take_profit_ratio().round_dp(4),
                    input_17: position.price_variance().round_dp(4),
                    input_18: position.atr_spread().round_dp(4),
